...
def train(train_loader, model, optimizer, epoch, save_path, writer):
    ...
    try:
        for i, (images, gts) in enumerate(train_loader, start=1):
            ...
            preds = model(images)
            loss = structure_loss(preds[0], gts) + structure_loss(preds[1], gts) + structure_loss(preds[2], gts) + structure_loss(preds[3], gts)
...
def val(test_loader, model):
    ...
    with torch.no_grad():
        for i, (image, gt) in enumerate(test_loader):
            ...
            pred = model(image)
            loss_total = structure_loss(pred[0], gt) + structure_loss(pred[1], gt) + structure_loss(pred[2], gt) + structure_loss(pred[3], gt)

            loss = loss_total * image.shape[0]
            losses += loss.item()  

            num += image.shape[0]
            pred_ = pred[-1].squeeze().sigmoid().data.cpu().numpy()
...
def main(args):
    # build the model
    model = Network(mode = args.mode, ratio_list = args.ratio_list).cuda()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ratio_list', type=list, default=[0.75, 0.75, 1], help='Selection ratio from shallow to deep layers')
    parser.add_argument('--save_path', type=str, default='/data/ll/Med_Seg/SINet/SINet_github/save_path', help='the path to save model, figure and log')
    parser.add_argument('--save_name', type=str, default='entro_751', help='the name of saved dir')
